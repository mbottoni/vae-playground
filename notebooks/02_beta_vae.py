...
def _():
    ...
    Beta-VAE modifies the standard ELBO by scaling the KL term with a coefficient $\beta$:

    $$\mathcal{L} = \mathbb{E}_{q_\phi(z|x)}[\log p_\theta(x|z)]
      - \beta \cdot D_\text{KL}(q_\phi(z|x) \| p(z))$$

    - $\beta = 1$: recovers the standard VAE.
    - $\beta > 1$: applies stronger pressure toward the prior, encouraging
      **disentangled** latent dimensions — each dimension tends to encode a
      single generative factor.

    Experiment with the $\beta$ slider below to see the disentanglement / reconstruction trade-off.
    """)
    return (mo,)
...
def _():
    ...
    from vae_playground.models import BetaVAE
    ...
    return (
        BetaVAE, Path, Trainer, compute_latent_stats, get_dataloader, np,
...
def _(mo):
    ...
    beta_slider = mo.ui.slider(0.1, 20.0, value=4.0, step=0.1, label="Beta (β)")
    ...
    """)
    return batch_size_slider, beta_slider, dataset_dd, epochs_slider, latent_dim_slider, lr_slider
...
@app.cell
def _(
    BetaVAE, Trainer, batch_size_slider, beta_slider, dataset_dd,
    ...
    mo.md(f"""
    **Training complete** on `{_trainer.device}`
    — β = {beta_slider.value}
...
def _(history, mo, plot_loss_curves):
    mo.ui.plotly(plot_loss_curves(history, title="Beta-VAE — Training Loss"))
    return
...
def _(model, mo, plot_reconstructions, torch, val_loader):
    ...
    mo.ui.plotly(plot_reconstructions(_x, _recon, n=10, title="Beta-VAE — Reconstructions"))
    return
...
def _(model, mo, plot_samples):
    ...
    mo.ui.plotly(plot_samples(_samples, n=16, title="Beta-VAE — Samples"))
    return
